 ...
 * See https://ca2.php.net/manual/en/language.pseudo-types.php#language.types.callback for examples
 * of callback formats.
...
/**
 * Group elements of an array by specified key
 * The returned array contains the found key values as keys and the correspondent values
 * 
 * @param array $data
 * @param string $key
 ...
 */
	public static function group($data, $key, $default='') {
	    ...
	    foreach ($data as $k => $v) {
	        if (isset($v[$key])) {
	            $groupBy = (empty($v[$key])) ? $default : $v[$key];
	            $result[$groupBy][] = $v;
	        }
	    }
...
/**
 * Group elements of an array by specified keys,
 * comparing them against additional key checks
 * 
 * @param array $data
 * @param mixed $keysToGroup
 * @param mixed $keysToCompare
 * @return array
 */
	public static function groupMulti($data, $keysToGroup, $keysToCompare) {
	    $keysToGroup = (array) $keysToGroup;
	    $keysToCompare = (array) $keysToCompare;
        ...
        foreach ($data as $item) {
            ...
            $keysPresent = true;
            foreach (array_merge($keysToGroup, $keysToCompare) as $keyToCheck) {
            ...
            if (!$result) {
                $result[$groupCounter] = array();
                foreach ($keysToGroup as $keyToGroup) {
                    $result[$groupCounter][$keyToGroup][] = $item[$keyToGroup];
                }
                foreach ($keysToCompare as $keyToCompare) {
                    $result[$groupCounter][$keyToCompare] = $item[$keyToCompare];
                }
            } else {

                $currGroupId = uniqid();
                foreach ($result as $groupId => $resultData) {
                    
                    $allKeysMatch = true;
                    foreach ($keysToCompare as $keyToCompare) {
                        if ($item[$keyToCompare] != $resultData[$keyToCompare]) {
                            $allKeysMatch = false;
                            break;
                        }
                    }
                ...
                foreach ($keysToGroup as $keyToGroup) {
                    $result[$currGroupId][$keyToGroup][] = $item[$keyToGroup];
                }
                foreach ($keysToCompare as $keyToCompare) {
                    $result[$currGroupId][$keyToCompare] = $item[$keyToCompare];
                }
            }
        }
    ...
    public static function prioritize($data, $needle) {
        $data = (array) $data;
        $priorityMap = array();
        
        $flatPrioritMap = Set::flatten($data);
        foreach ($flatPrioritMap as $rootMap => $rootString) {
            $rootMapParts = array_reverse(explode('.', $rootMap));
            array_shift($rootMapParts);
            if ($needle == $rootString) {
                $priorityMap = array_merge(array($rootString), $rootMapParts);
                break;
            } elseif (in_array($needle, $rootMapParts)) {
                $currKeyPos = array_search($needle, $rootMapParts);
                $priorityMap = array_slice($rootMapParts, $currKeyPos);
            }
        }
