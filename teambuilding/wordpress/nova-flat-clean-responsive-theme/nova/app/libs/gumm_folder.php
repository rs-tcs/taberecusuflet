 ...
 */
	function read($sort = true, $exceptions = false, $fullPath = false) {
		...
		while (false !== ($item = readdir($dir))) {
			...
			$path = GummFolder::addPathElement($this->path, $item);
			if (is_dir($path)) {
	...
	function _findRecursive($pattern, $sort = false) {
		...
		foreach ($files as $file) {
			if (preg_match('/^' . $pattern . '$/i', $file)) {
				$found[] = GummFolder::addPathElement($this->path, $file);
			}
		}
		$start = $this->path;

		foreach ($dirs as $dir) {
			$this->cd(GummFolder::addPathElement($start, $dir));
 ...
 */
	function normalizePath($path) {
		return GummFolder::correctSlashFor($path);
	}
 ...
 */
	function slashTerm($path) {
		if (GummFolder::isSlashTerm($path)) {
			return $path;
		}
		return $path . GummFolder::correctSlashFor($path);
	}
 ...
 */
	function inCakePath($path = '') {
		$dir = substr(GummFolder::slashTerm(ROOT), 0, -1);
		$newdir = $dir . $path;
 ...
 */
	function inPath($path = '', $reverse = false) {
		$dir = GummFolder::slashTerm($path);
		$current = GummFolder::slashTerm($this->pwd());
 ...
 */
	function dirsize() {
		$size = 0;
		$directory = GummFolder::slashTerm($this->path);
 ...
 */
	function delete($path = null) {
		...
		$path = GummFolder::slashTerm($path);
		if (is_dir($path) === true) {
		...
		if ($handle = @opendir($fromDir)) {
			while (false !== ($item = readdir($handle))) {
				if (!in_array($item, $exceptions)) {
					$from = GummFolder::addPathElement($fromDir, $item);
					$to = GummFolder::addPathElement($toDir, $item);
					if (is_file($from)) {
						if (copy($from, $to)) {
