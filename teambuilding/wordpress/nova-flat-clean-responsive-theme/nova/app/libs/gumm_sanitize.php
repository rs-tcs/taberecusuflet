 ...
 * - quotes (int) see https://php.net/manual/en/function.htmlentities.php
 ...
 */
	public static function stripAll($str) {
		$str = GummSanitize::stripWhitespace($str);
		$str = GummSanitize::stripImages($str);
		$str = GummSanitize::stripScripts($str);
		return $str;
	}
 ...
 *
 * Ex.`$clean = GummSanitize::stripTags($dirty, 'b', 'p', 'div');`
 *
	...
	public static function clean($data, $options = array()) {
		...
		if (is_array($data)) {
			foreach ($data as $key => $val) {
				$data[$key] = GummSanitize::clean($val, $options);
			}
			return $data;
		} else {
			...
			if ($options['encode']) {
				$data = GummSanitize::html($data, array('remove' => $options['remove_html']));
			}
			if ($options['dollar']) {
			...
			if ($options['escape']) {
				$data = GummSanitize::escape($data, $options['connection']);
			}
